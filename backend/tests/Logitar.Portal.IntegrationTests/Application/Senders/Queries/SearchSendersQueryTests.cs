...
public class SearchSendersQueryTests : IntegrationTests
{
  ...
  {
    Sender notInSearch = new(new Email(Faker.Internet.Email()), new ReadOnlySendGridSettings(SendGridHelper.GenerateApiKey()), id: SenderId.NewId(TenantId))
    {
      DisplayName = new DisplayName(Faker.Name.FullName())
    };
    notInSearch.Update();
    Sender notInIds = new(new Email(Faker.Internet.Email()), new ReadOnlySendGridSettings(SendGridHelper.GenerateApiKey()), id: SenderId.NewId(TenantId))
    {
      DisplayName = new DisplayName(string.Join(' ', Faker.Name.FirstName(), "Sender", Faker.Name.LastName()))
    };
    notInIds.Update();
    Sender sender1 = new(new Email(Faker.Internet.Email()), new ReadOnlySendGridSettings(SendGridHelper.GenerateApiKey()), id: SenderId.NewId(TenantId))
    {
      DisplayName = new DisplayName(string.Join(' ', Faker.Name.FirstName(), "Sender", Faker.Name.LastName()))
    };
    sender1.Update();
    Sender sender2 = new(new Email(Faker.Internet.Email()), new ReadOnlySendGridSettings(SendGridHelper.GenerateApiKey()), id: SenderId.NewId(TenantId))
    {
      DisplayName = new DisplayName(string.Join(' ', Faker.Name.FirstName(), "Sender", Faker.Name.LastName()))
    };
    sender2.Update();
