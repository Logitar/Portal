...
namespace Logitar.Portal.Core.Realms.Commands
{
  internal class UpdateRealmCommandHandler : IRequestHandler<UpdateRealmCommand, RealmModel>
  {
    ...
    public async Task<RealmModel> Handle(UpdateRealmCommand request, CancellationToken cancellationToken)
    {
      ...
      realm.Update(payload.DisplayName, payload.Description, defaultLocale, payload.Url,
        payload.RequireConfirmedAccount, payload.RequireUniqueEmail, payload.UsernameSettings, payload.PasswordSettings,
        passwordRecoverySenderId: null, passwordRecoveryTemplateId: null, // TODO(fpion): implement
