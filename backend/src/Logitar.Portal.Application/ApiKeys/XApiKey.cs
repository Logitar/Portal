...
internal record XApiKey
{
  ...
  private XApiKey(ApiKeyId id, string secret)
  {
  ...
  public static XApiKey Decode(string encoded)
  {
    string[] values = encoded.Split(Separator);
    if (values.Length != 3 || values.First() != Prefix)
    {
      throw new ArgumentException($"The value '{encoded}' is not a valid refresh token.", nameof(encoded));
    }
