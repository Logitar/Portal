...
internal class SignOutUserCommandHandler : IRequestHandler<SignOutUserCommand, User?>
{
  ...
  public SignOutUserCommandHandler(IApplicationContext applicationContext, ISessionRepository sessionRepository, IUserQuerier userQuerier,
  ...
  public async Task<User?> Handle(SignOutUserCommand command, CancellationToken cancellationToken)
  {
    ...
    if (user == null /*|| user.TenantId?.Value != _applicationContext.Realm?.Id*/) // TODO(fpion): ensure User is in current Realm
    {
      return null;
    }
