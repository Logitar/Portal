...
internal class UserFacade : IUserService
{
  ...
  public UserFacade(IRequestPipeline pipeline)
  {
    _pipeline = pipeline;
  }
  ...
  {
    return await _pipeline.ExecuteAsync(new ReadUserQuery(id, uniqueName, identifier), cancellationToken);
  }
  ...
  {
    return await _pipeline.ExecuteAsync(new ResetUserPasswordCommand(id, payload), cancellationToken);
  }
  ...
  {
    return await _pipeline.ExecuteAsync(new SaveUserIdentifierCommand(id, key, payload), cancellationToken);
  }
