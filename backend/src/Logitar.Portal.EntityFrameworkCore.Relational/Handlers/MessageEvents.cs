  ...
  INotificationHandler<MessageSucceeded>
{
  ...
  public MessageEvents(IdentityContext identityContext, PortalContext portalContext)
  {
  ...
  public async Task Handle(MessageFailed @event, CancellationToken cancellationToken)
  {
    MessageEntity? message = await _context.Messages
      .SingleOrDefaultAsync(x => x.StreamId == @event.StreamId.Value, cancellationToken)
      ?? throw new InvalidOperationException($"The message entity 'StreamId={@event.StreamId}' could not be found.");
