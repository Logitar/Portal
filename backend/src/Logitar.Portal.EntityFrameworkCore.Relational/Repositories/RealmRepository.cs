...
{
  private static readonly string _aggregateType = typeof(RealmAggregate).GetNamespaceQualifiedName();
  ...
  public async Task<RealmAggregate?> LoadAsync(RealmId id, bool includeDeleted, CancellationToken cancellationToken)
    => await LoadAsync(id, version: null, includeDeleted, cancellationToken);
  public async Task<RealmAggregate?> LoadAsync(RealmId id, long? version, bool includeDeleted, CancellationToken cancellationToken)
    => await base.LoadAsync<RealmAggregate>(id.AggregateId, version, includeDeleted, cancellationToken);
  ...
  public async Task<IEnumerable<RealmAggregate>> LoadAsync(bool includeDeleted, CancellationToken cancellationToken)
    => await base.LoadAsync<RealmAggregate>(includeDeleted, cancellationToken);

  public async Task<IEnumerable<RealmAggregate>> LoadAsync(IEnumerable<RealmId> ids, CancellationToken cancellationToken)
    => await LoadAsync(ids, includeDeleted: false, cancellationToken);
  public async Task<IEnumerable<RealmAggregate>> LoadAsync(IEnumerable<RealmId> ids, bool includeDeleted, CancellationToken cancellationToken)
  {
    IEnumerable<AggregateId> aggregateIds = ids.Select(id => id.AggregateId);
    return await base.LoadAsync<RealmAggregate>(aggregateIds, includeDeleted, cancellationToken);
  }

  public async Task<RealmAggregate?> LoadAsync(UniqueSlugUnit uniqueSlug, CancellationToken cancellationToken)
  {
    string uniqueSlugNormalized = uniqueSlug.Value.ToUpper();

    IQuery query = _sqlHelper.QueryFrom(EventDb.Events.Table)
      .Join(PortalDb.Realms.AggregateId, EventDb.Events.AggregateId,
    new OperatorCondition(EventDb.Events.AggregateType, Operators.IsEqualTo(_aggregateType))
    )
