...
internal class UserQuerier : IUserQuerier
{
  ...
  public async Task<User> ReadAsync(RealmModel? realm, UserAggregate user, CancellationToken cancellationToken)
  {
  ...
  public async Task<User?> ReadAsync(RealmModel? realm, UserId id, CancellationToken cancellationToken)
    => await ReadAsync(realm, id.ToGuid(), cancellationToken);
  public async Task<User?> ReadAsync(RealmModel? realm, Guid id, CancellationToken cancellationToken)
  {
  ...
  public async Task<User?> ReadAsync(RealmModel? realm, string uniqueName, CancellationToken cancellationToken)
  {
    string? tenantId = realm?.GetTenantId().Value;
  ...
  public async Task<IEnumerable<User>> ReadAsync(RealmModel? realm, IEmail email, CancellationToken cancellationToken)
  {
    string? tenantId = realm?.GetTenantId().Value;
  ...
  public async Task<User?> ReadAsync(RealmModel? realm, CustomIdentifier identifier, CancellationToken cancellationToken)
  {
    string? tenantId = realm?.GetTenantId().Value;
  ...
  public async Task<SearchResults<User>> SearchAsync(RealmModel? realm, SearchUsersPayload payload, CancellationToken cancellationToken)
  {
  ...
  private async Task<User> MapAsync(UserEntity user, RealmModel? realm, CancellationToken cancellationToken = default)
    => (await MapAsync([user], realm, cancellationToken)).Single();
  private async Task<IEnumerable<User>> MapAsync(IEnumerable<UserEntity> users, RealmModel? realm, CancellationToken cancellationToken = default)
  {
