...
public class UserNotFoundException : InvalidCredentialsException
{
  ...
  public UserNotFoundException(RealmAggregate? realm, string uniqueName)
    : base(BuildMessage(realm, uniqueName))
  {
  ...
  private static string BuildMessage(RealmAggregate? realm, string uniqueName)
  {
    ...
    message.Append("Realm: ").Append(realm?.ToString() ?? "<null>").AppendLine();
