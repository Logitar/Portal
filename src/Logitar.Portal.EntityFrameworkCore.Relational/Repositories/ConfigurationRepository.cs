...
internal class ConfigurationRepository : EventSourcing.EntityFrameworkCore.Relational.AggregateRepository, IConfigurationRepository
{
  ...
  public ConfigurationRepository(ICacheService cacheService, IEventBus eventBus, EventContext eventContext, IEventSerializer eventSerializer)
  ...
  public async Task<ConfigurationAggregate?> LoadAsync(long? version, CancellationToken cancellationToken)
    => await LoadAsync<ConfigurationAggregate>(ConfigurationAggregate.UniqueId, version, cancellationToken);
